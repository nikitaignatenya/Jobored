...
import Style from "../styles/Vacancy.module.scss";
...
function Vacancy() {
  ...
  const { id } = useParams();
  const array = vacancies.filter((el) => id == el.id);
  console.log(array);
    ...
    <div>
      ...
      <div className={Style.mainContainer}>
        <div className={Style.firstSection}>
          <h1>{array[0].title}</h1>
          <h3>
            з/п {array[0].salary} <span>{array[0].schedule}</span>
          </h3>
          <div className={Style.locationContainer}>
            <img src={location}></img>
            <p>{array[0].location}</p>
          </div>
        </div>
        <div className={Style.secondContainer}>
          <div className={Style.responsibilitiesContainer}>
            <h3>Обязанности:</h3>
            <ul>
              {array[0].responsibilities.map((el) => (
                <li>{el}</li>
              ))}
            </ul>
          </div>
